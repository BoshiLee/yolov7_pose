...
if __name__ == '__main__':
    # gs load a graph
    graph = gs.import_onnx(onnx.load("onnx_inference/yolov7-w6-pose.onnx"))
    ...
    inputs = [tensors["754"].to_variable(dtype=np.float32),
              tensors["807"].to_variable(dtype=np.float32),
              tensors["860"].to_variable(dtype=np.float32),
              tensors["913"].to_variable(dtype=np.float32)]
    ...
    new_node = gs.Node(op="Concat", name="concat_transpose", inputs=inputs, outputs=[modified_output])
